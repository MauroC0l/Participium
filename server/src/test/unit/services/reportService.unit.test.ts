...
describe('ReportService additional unit tests', () => {
  ...
  describe('getAllReports', () => {
    ...
    it('returns filtered reports for non-officer users', async () => {
      ...
      const reports = [
        { id: 1, status: ReportStatus.PENDING_APPROVAL } as any,
        { id: 2, status: ReportStatus.RESOLVED } as any,
      ];
      jest.spyOn(reportRepository, 'findAllReports').mockResolvedValue(reports as any);
      jest.spyOn(mapperService, 'mapReportEntityToReportResponse').mockImplementation(r => ({ id: r.id } as any));

      const result = await reportService.getAllReports(user.id);
      expect(result.map(r => (r as any).id)).toEqual([2]);
    });

    it('returns all including pending for officer users', async () => {
      const user = makeUser('Municipal Public Relations Officer');
      (userRepository.findUserById as jest.Mock).mockResolvedValue(user);
